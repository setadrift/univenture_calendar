...
(() => {
  ...
  var FakeComponent = class extends k {
    render() {
      return y("div", {});
    }
    componentDidMount() {
  ...
  var PureComponent = class extends k {
  ...
  var ContentContainer = class extends k {
    constructor() {
      super(...arguments);
    ...
    constructor(el, optionOverrides = {}) {
      ...
      this.handleRenderRequest = () => {
        if (this.isRendering) {
          ...
          flushSync(() => {
            B(y(CalendarRoot, { options: currentData.calendarOptions, theme: currentData.theme, emitter: currentData.emitter }, (classNames, height, isHeightAuto, forPrint) => {
    ...
    const events = [...scripts].reduce((acc, script) => {
      if (script.textContent) {
        ...
        const event = {
          ...
          // parse the session number as an integer
        };
        acc.push(event);
      }
      return acc;
  ...
  var updateCalendar = (calendar, session) => {
    const events = getEvents().filter((event) => event.session === session);
    calendar.getEventSources().forEach((source) => source.remove());
    calendar.addEventSource(events);
  };
  var preferredDatePicker1 = esm_default("#Preferred-Date-1", {
  ...
  var preferredDatePicker2 = esm_default("#Preferred-Date-2", {
    dateFormat: "Y-m-d H:i",
    enableTime: true,
    time_24hr: true,
    mode: "multiple",
  ...
  function updateSelectedDatesList(selectedDates, session) {
    ...
    selectedDates.forEach((date, index4) => {
      ...
      deleteButton.addEventListener("click", () => {
        if (session === 1) {
          preferredDatePicker1.selectedDates.splice(index4, 1);
          preferredDatePicker1.redraw();
        } else {
          preferredDatePicker2.selectedDates.splice(index4, 1);
    ...
    const listContainer = document.querySelector(`#selected-dates-list-${session}`);
    if (listContainer) {
  ...
  var form = document.getElementById("custom-availability-form");
  if (form) {
    form.addEventListener("submit", (event) => {
      ...
      const name = formElements.namedItem("name");
      const email = formElements.namedItem("email");
      const userAvailabilities2 = {
        name: name.value,
        email: email.value,
        session1: session1SelectedDatesStrings,
        session2: session2SelectedDatesStrings
      };
      console.log(userAvailabilities2);
